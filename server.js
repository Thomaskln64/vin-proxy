...
// WICHTIG: Auf Render unbedingt setzen: PUBLIC_BASE_URL=https://fzb-24-api.onrender.com
...
function summarizeStolen(stolenJson) {
  ...
  const anyStolen = rows.some(
    (r) => String(r.status).toLowerCase() === "stolen"
  );
  return {
    available: true,
...
async function buildPremiumReport(vin, email = null) {
  ...
  const report = {
    ...
    checks: {
      stolen: stolenR.ok
        ? summarizeStolen(stolenR.json)
        : { available: false, status: "unavailable" },
      market_value: valueR.ok
        ? summarizeMarketValue(valueR.json)
...
function renderReportHtml(report) {
  ...
  if (stolenStatus === "not-stolen")
    verdict = { label: "OK", color: "ok", text: "Kein Treffer im EU-Diebstahlcheck gefunden." };
  if (stolenStatus === "stolen")
    verdict = { label: "Achtung", color: "bad", text: "Treffer im Diebstahlcheck. Bitte unbedingt prüfen." };

  const title = `${v.make || ""} ${v.model || ""} (${v.year || "—"})`.trim();
  const marketText = market?.available
    ? "Verfügbar"
    : `Nicht verfügbar (${market?.reason || "keine Daten"})`;

  const stolenDetailsRows =
    (report.checks?.stolen?.details || [])
      .map(
        (r) =>
          `<tr><td>${escapeHtml(r.source)}</td><td>${escapeHtml(r.status)}</td></tr>`
      )
...
app.get("/api/report/:vin", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      locked_sections: [
        {
          title: "Diebstahlcheck (EU)",
          hint: "Prüfung über mehrere EU-Datenbanken (Details im PDF).",
        },
        {
          title: "Marktwert",
          hint: "Marktwert-Indikator (wenn ausreichende Marktdaten vorhanden).",
        },
        {
          title: "Technische Daten",
          hint: "Maße, Gewicht, Bremsen, Lenkung, Räder, CO₂ u.v.m.",
        },
        {
          title: "Hinweise & Datenquellen",
          hint: "Transparente Erklärung, was geprüft wurde und was nicht.",
        },
      ],
    });
  } catch (err) {
    ...
    return res
      .status(500)
      .json({ success: false, error: "server_error", details: err.message });
  }
});
...
// (Alt/Manuell) erstellt PDF + Download-Link (wie früher)
...
// (Neu) Wix Automation Webhook nach Zahlung
// Erstmal nur "OK empfangen" – PDF+Mail bauen wir als nächsten Schritt sauber.
app.post("/api/order-from-wix", async (req, res) => {
  try {
    ...
    // Hier kommt als nächster Schritt:
    // 1) Payment verifizieren (orderId/purchaseFlowId)
    // 2) Premium Report bauen
    // 3) PDF rendern
    // 4) Mail senden + Link
    if (DEBUG) console.log("✅ order-from-wix:", { purchaseFlowId, email, vin });

    return res.status(200).json({
      success: true,
      received: { purchaseFlowId, email, vin: sanitizeVin(vin) },
